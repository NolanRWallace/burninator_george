...
while run:
    ...
    if keys[pygame.K_LEFT] and hero.x > hero.vel - hero.width:
    ...
    if boss.visible:    
        while j == 23:
            ...
            if len(boss_fireballs) < 100:
                # pygame.mixer.Sound.play(fireball_sound)
                boss_fireballs.append(Projectile(round(boss.x + boss.width //2), round(boss.y + boss.height //2), (235,122,52), 10, boss_direction))
            j += 1
